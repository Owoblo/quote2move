-- Create a custom type for user roles in movsense schema
CREATE TYPE movsense.user_role AS ENUM ('admin', 'manager', 'rep');

-- Create the companies table in movsense schema
CREATE TABLE movsense.companies (
...
CREATE INDEX idx_companies_owner_id ON movsense.companies(owner_id);

-- Create the profiles table to store user-specific data in movsense schema
CREATE TABLE movsense.profiles (
...
CREATE INDEX idx_profiles_company_id ON movsense.profiles(company_id);
...
CREATE INDEX idx_quotes_company_id ON movsense.quotes(company_id);

-- Create the pricing_rules table in movsense schema
CREATE TABLE movsense.pricing_rules (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_pricing_rules_company_id ON movsense.pricing_rules(company_id);
